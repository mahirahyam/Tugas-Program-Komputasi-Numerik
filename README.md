...
```
Import library `numpy` dan `sympy` dari Python
```
def compute_diff_table(y_vals):
    ...
    return diff
```
Fungsi `compute_diff_table` membentuk tabel selisih hingga orde ke-n.
```
...
print(f"\nf({xt}) = {hasil}")
```
Fungsi `stirling_interpolation` berfungsi untuk melakukan operasi interpolasi stirling sesuai soal dan mencetak hasil dari f(16).
```
print(f"Et ={abs((yt - hasil)/yt) * 100 : .2f}")
```
Print hasil persentase _error true_.
